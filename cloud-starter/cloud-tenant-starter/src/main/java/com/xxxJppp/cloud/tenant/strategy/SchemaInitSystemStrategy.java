...
import com.xxxJppp.cloud.common.constant.StrPool;
import com.xxxJppp.cloud.common.exception.BusinessException;
import com.xxxJppp.cloud.tenant.mapper.InitDbMapper;
 ...
 * <p>
 * 初始化规则：
 * /src/main/resources/sql 路径存放8个sql文件 (每个库对应一个文件)
...
public class SchemaInitSystemStrategy implements InitSystemStrategy {
     ...
     * 可能不同的服务，会连接不同的库
     */
    private final static List<String> INIT_DATABASE_LIST = Arrays.asList(CommonConstant.DEFAULT_DATA_BASE_DATABASE);
    @Autowired
    private DataSource dataSource;
    @Autowired
    private InitDbMapper initDbMapper;

    @Value("${cloud.data.database}")
    ...
    public boolean reset(String tenant) {
        ...
        try {
            ...
            String tenantDatabasePrefix = tenantProperties.getTenantDatabasePrefix();
    ...
    public void initTables(String tenant) {
        ...
        try {
            ...
            log.error("初始化表失败", e);
            throw new BusinessException("初始化表失败");
        } finally {
            try {
                if (runner != null) {
                    runner.closeConnection();
    ...
    public void resetDatabase() {
        ...
        try {
            ...
            throw new BusinessException("切换为默认数据源失败");
        } finally {
            try {
                if (runner != null) {
                    runner.closeConnection();
...
//            设置分隔符 runner.setDelimiter(";");
    ...
    public boolean delete(List<String> tenantCodeList) {
        ...
        tenantCodeList.forEach((tenant) -> {
            String databasePrefix = tenantProperties.getTenantDatabasePrefix();
